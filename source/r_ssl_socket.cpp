...
{
    return static_cast<r_raw_socket*>(ctx)->raw_send(buf, len);
}

static int _mbedtls_recv(void* ctx, unsigned char* buf, size_t len)
{
    return static_cast<r_raw_socket*>(ctx)->raw_recv(buf, len);
}
...
void r_ssl_socket::send(const void* buf, size_t len)
{
    size_t total = 0;
    while (total < len) {
        int ret = raw_send((const uint8_t*)buf + total, len - total);
        if (ret < 0)
            R_THROW(("r_ssl_socket::send() failed"));
        total += ret;
    }
}

void r_ssl_socket::recv(void* buf, size_t len)
{
    size_t total = 0;
    while (total < len) {
        int ret = raw_recv((uint8_t*)buf + total, len - total);
        if (ret <= 0)
